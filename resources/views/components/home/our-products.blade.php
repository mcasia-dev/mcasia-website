...
@php
    ...
    $productHighlightsData = collect(data_get($section, 'highlights', []))
        ->map(function ($item) {
            return [
                'image' => filled($item['image'] ?? null) ? Storage::disk('public')->url($item['image']) : null,
    ...
    if (empty($productHighlightsData)) {
        $productHighlightsData = $fallbackProductHighlights;
    }

    $title = data_get($section, 'title', 'Our Products');
    $description = data_get($section, 'description', 'We offer a carefully curated portfolio of authentic Asian food and beverage products, spanning Cooking Essentials, Frozen Meat & Seafood, Beverages, and Snacks, bringing genuine flavors and quality to modern consumers.');
    $buttonLabel = data_get($section, 'button_label', 'All Products');
    $buttonUrl = data_get($section, 'button_url', '/products/cooking-essentials/cooking-essentials-canned-goods');
@endphp
    ...
    <div class="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-2 md:gap-8 xl:gap-2 items-center">
        <div data-aos="fade-right" data-aos-duration="700" class="w-full max-w-[450px] mx-auto lg:mx-0">
            <div class="relative rounded-2xl overflow-hidden shadow-lg">
                <img
                    id="productHeroImage"
                    src="{{ $productHighlightsData[0]['image'] ?? '' }}"
                    alt="{{ $productHighlightsData[0]['label'] ?? '' }}"
                    title="{{ $productHighlightsData[0]['label'] ?? '' }}"
                    loading="lazy"
                    decoding="async"
                    fetchpriority="high"
        ...
        <div data-aos="fade-left" data-aos-duration="700" data-aos-delay="100"
             class="flex flex-col mt-10 md:mt-0 font-onest">
            <h2 class="text-3xl sm:text-5xl font-bold text-gray-900 mb-4">{{ $title }}</h2>
            <div class="mb-8 text-sm md:text-base text-justify">
