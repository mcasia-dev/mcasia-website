...
<style>
    .recipe-page {
        background:
            radial-gradient(1000px 500px at 5% -5%, rgba(220, 38, 38, 0.12), transparent 60%),
    ...
    <section class="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10 lg:py-12 space-y-6">
        <article class="recipe-surface rounded-3xl p-4 sm:p-6">
            <div class="media-shell rounded-2xl overflow-hidden">
                @if(!empty($recipe['videoEmbed']))
                    <video class="w-full rounded-2xl" autoplay playsinline controls>
                        <source src="{{ $recipe['videoEmbed'] }}" type="video/mp4">
                    </video>
                @else
                    <img src="{{ $recipe['image'] }}" class="w-full h-72 sm:h-96 object-cover rounded-2xl" alt="{{ $recipe['title'] }}">
                @endif
            </div>
        </article>
        ...
        <article class="recipe-surface rounded-3xl p-5 sm:p-7 lg:p-8">
            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-5">
                ...
                <a href="{{ route('recipes') }}" class="inline-flex items-center justify-center gap-2 h-11 px-4 rounded-xl border border-gray-300 text-sm sm:text-base text-gray-700 hover:text-red-600 hover:border-red-300 transition-colors">
