...
<style>
    ...
    .recipes-page {
        background:
            radial-gradient(950px 460px at 6% -6%, rgba(220, 38, 38, 0.1), transparent 60%),
    ...
    <section id="recipesSection" class="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10 lg:py-12">
        <article class="recipes-surface rounded-3xl p-5 sm:p-7 mb-6 sm:mb-8">
            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div>
                    ...
                    <h2 class="text-3xl sm:text-4xl font-bold text-gray-900">Recipes</h2>
                </div>
                <div class="flex items-center gap-3">
                    <span class="inline-flex items-center h-10 px-4 rounded-xl border border-red-100 bg-red-50 text-red-600 font-semibold text-sm">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            @forelse($recipes as $recipe)
                <a href="{{ route('recipes.show', ['slug' => $recipe['slug']]) }}" class="recipe-card rounded-2xl overflow-hidden text-left block">
                    <div class="recipe-thumb">
                        <img src="{{ $recipe['image'] }}" class="recipe-image w-full h-56 sm:h-60 object-cover" alt="{{ $recipe['title'] }}">
                    </div>
